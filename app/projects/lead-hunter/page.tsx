...
export default function LeadHunterPage() {
  return <main>
    ...
    <section className="case-metrics shell" aria-label="Показатели Lead Hunter"><article><small>За последние 24 часа</small><strong>79</strong><span>новых уникальных объявлений</span></article><article><small>В базе</small><strong>848</strong><span>уникальных лидов</span></article><article><small>Одобрено AI</small><strong>42</strong><span>кандидата за период работы</span></article><article><small>Доставлено</small><strong>37</strong><span>карточек в Telegram</span></article></section>
    <section className="case-story shell"><div className="case-story-title"><span>01 / Задача</span><h2>Не искать заказы.<br/><em>Получать лучшие.</em></h2></div><div className="case-copy"><p>На фриланс-площадках много нерелевантных, платных или слишком конкурентных объявлений. Ручная проверка каждой карточки съедает время ещё до начала работы.</p><p>За последние 24 часа Lead Hunter собрал 79 новых уникальных объявлений. До Telegram доходит лишь небольшая часть — это намеренно: система жёстко отсекает платные отклики, неподходящие направления, перегретую конкуренцию и задания без понятной экономики.</p><p>Поэтому число отправленных карточек не показывает максимальную производительность парсера. Оно показывает строгость отбора: в ленту попадают только заявки, которые стоит открыть и оценить человеку.</p></div></section>
